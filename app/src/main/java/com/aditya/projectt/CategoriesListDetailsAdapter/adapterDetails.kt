package com.aditya.projectt.CategoriesListDetailsAdapter
...
class adapterDetails(val requireContext: Context, val listOfUserData: ArrayList<homeData>) :
    RecyclerView.Adapter<adapterDetails.myViewHolder>() {
    ...
    override fun onBindViewHolder(holder: myViewHolder, position: Int) {
        ...
        holder.bookBtn.setOnClickListener {
            if (chLike == true) {
                chLike = false
                holder.bookBtn.setColorFilter(Color.RED)
            } else {
                holder.bookBtn.setColorFilter(Color.GRAY)
                chLike = true
            }
